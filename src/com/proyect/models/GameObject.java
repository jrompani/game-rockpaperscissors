package com.proyect.models;

public enum GameObject {

	ROCK, PAPER, SCISSORS;

	private GameObject beats;
	
	static {
		ROCK.beats = SCISSORS;
		PAPER.beats = ROCK;
		SCISSORS.beats = PAPER;
	}
	
	
	private int[][] comparison = {
			// rock paper scissors
			/* rock */ { 0, -1, 1, }, /* paper */ { 1, 0, -1, }, /* scissors */ { -1, 1, 0, } };

	private static String[][] compareText = { { "Rock matches rock", "Rock gets wrapped", "Rock smashes scissors" },
			{ "Paper wraps rock", "Paper matches paper", "Paper gets cut" },
			{ "Scissors get smashed", "Scissors cut paper", "Scissors match scissors" } };

	public String compareText(GameObject object) {
		return compareText[this.ordinal()][object.ordinal()];
	}
	
	public boolean beats(GameObject other) {
		return this.beats == other;
	}

}
